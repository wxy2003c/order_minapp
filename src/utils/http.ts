...
/**
 * 收集签名用的 query 参数：先取 URL 内联参数，再用 config.params 覆盖。
 * 不走 axios.getUri —— 顶层 axios 不支持 {serialize:fn} 格式的 paramsSerializer，
 * 会导致 params 无法序列化、签名 normalizedParams 为空、服务端验签失败。
 */
function collectUrlQuery(config: InternalAxiosRequestConfig): Record<string, unknown> {
  const out: Record<string, unknown> = {}
  // 先提取 url 里可能内联的 ?foo=bar 参数
  try {
    const urlStr = String(config.url || '')
    const qi = urlStr.indexOf('?')
    if (qi >= 0) new URLSearchParams(urlStr.slice(qi + 1)).forEach((v, k) => { out[k] = v })
  } catch { /* ignore */ }
  // config.params 覆盖（这是 applyGlobalQueryAndLang 设置的主要来源）
