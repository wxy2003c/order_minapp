...
export const useStaffDeeplinkStore = defineStore('staffDeeplink', () => {
  ...
  /**
   * 从订单列表点「+」进入创建订单时调用：
   * 将 `platformUid`（manage 链接的 uid）转移为 `customerTelegramId`（客户 id），
   * 同时清空 `platformUid`，使 HTTP `user_id` 回退到 SDK（当前打开者 Telegram）。
   * 若无 `platformUid`（无深链场景）则静默跳过。
   */
  function prepareCreateOrderFromList() {
    const uid = String(platformUid.value).trim()
    if (!uid) return
    customerTelegramId.value = uid
    platformUid.value = ''
    syncSessionFromState()
  }
