...
const API_BASE_URL = ('https://chain.wheelsline.com/api/v1').trim();
/** 明文密钥；须与 DB 解密后的 signingSecret 一致，勿把 migrations 里加密后的密文当 secret */
const API_SECRET_RAW = ('YzBjNDBkOTE4MFjNWE2Njk5OTMzYTExYzRkIiwidGFnIjoiIn0').trim();
const API_KEY = ('telegram_key').trim();


/**
 * 订单 REST 的 `lang` 查询参数。
 * 此处禁止 import `@lib/langPack`（含动态 import）：orderHttp → orderSignedHttp 会在启动极早期加载本模块，
 * 任何对 langPack 的引用都会拖入巨型 chunk，与首屏/SW 争抢，表现为 Network 卡在 apiLang 之后。
 * 签名请求用浏览器语言即可；若需与 UI 完全一致，可在业务层在 language_apply 后改请求策略。
 */
export function getOrdersApiLang(): string {
    if (typeof document !== 'undefined' && document.documentElement.lang) {
        const code = document.documentElement.lang.split('-')[0].trim();
        if (code) {
            return code;
        }
    }
    ...
    if (typeof navigator !== 'undefined' && navigator.language) {
        return navigator.language.split('-')[0].trim().toLowerCase() || 'en';
    }
